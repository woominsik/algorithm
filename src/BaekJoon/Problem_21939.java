...
public class Problem_21939 {
    public static void main(String[] args) {
        ...
        TreeSet<Main.Problem> ts = new TreeSet<>();
        Map<Integer,Integer> map = new HashMap<>();
        for (int i = 0; i < n; i++) {
            int nowidx = sc.nextInt();
            int nowlevel = sc.nextInt();
            ts.add(new Main.Problem(nowidx, nowlevel));
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (command.equals("add")) {
                ...
                if (command.equals("recommend")) {
                    ...
                    int nowidx = sc.nextInt();
                    ts.remove(new Main.Problem(nowidx,map.get(nowidx)));
                    map.remove(nowidx);
                }
            }
        }
    }
    public static class Problem implements Comparable<Main.Problem> {
        int idx;
        int level;
